...
1. **Understand Scope**:
   - Read `/.opencode/artifacts/current_run/state.yaml` and verify that the active phase is `Documentation`.
...
3. **Generate Walkthrough**:
   - Read the template `/.opencode/artifacts/templates/walkthrough_template.md`.
   - Generate `/.opencode/artifacts/current_run/walkthrough.md` detailing the changes accomplished and verification/testing results (or manual audit findings if bypassed).
