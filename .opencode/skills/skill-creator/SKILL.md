---
name: skill-creator
description: Empower the agent to convert chat histories, repetitive instructions, or specific procedures into highly structured, reusable skills under the multi-agent Harness system using the automated skill creator CLI and associated automation scripts.
---
...
1. **Create the Skill**: Run the following command in the workspace root:
   ```bash
   python3 .opencode/skills/scripts/skill_creator_cli.py --name "[Proper Capitalized Skill Name]" --description "[Concise, clear purpose of the skill]" [--profile <profile_name>]
   ```
   *Example*: `python3 .opencode/skills/scripts/skill_creator_cli.py --name "Git Management" --description "Establish secure git protocols."`

2. **Draft the Content**:
   - Open the newly created `SKILL.md` at `.opencode/skills/<skill-folder-name>/SKILL.md`.
...
3. **Implement Automation Scripts (ONLY when explicitly requested)**:
   ...
   - Save it inside the skill's assets folder: `.opencode/skills/<skill-folder-name>/assets/<script_name>.py`
