...
1. **Initialize State & Checklist**:
   - Read `/.harness/artifacts/current_run/state.yaml` and verify that the active phase is `Implementation` and the active profile is `python-developer`.
   ...
   - Load project configuration from `/.harness/config.yaml` to get linter (`lint_command` e.g. `flake8`) and test (`test_command` e.g. `pytest`) details.
...
3. **Execute Python Skills**:
   - Run profile-specific Python analysis and design skills:
     - Invoke `/.harness/skills/python_testing.md` to ensure correct pytest fixture usage, patch mocking, and exception assertions.
     - Invoke `/.harness/skills/python_clean_architecture.md` to enforce SOLID principles and dependency injection constructed with constructor inputs.
     - Invoke `/.harness/skills/python_performance_optimization.md` to optimize memory footprints with slots or lazy evaluation generator models if handling large collections.
